...
public:
    ...
    */
    ...
    void setSeconds(int32_t seconds) { _seconds = seconds; }
...
public:
    ...
    TimeOfDay(uint8_t hh, uint8_t mm, uint8_t ss)
    {
        this->hh = hh;
        this->mm = mm;
        this->ss = ss;
    ...
    bool operator<(const TimeOfDay &right)
    {
        TimeOfDay current(hh, mm, ss);
        return addTime(current) < addTime(right);
    }
    bool operator>(const TimeOfDay &right)
    {
        TimeOfDay current(hh, mm, ss);
